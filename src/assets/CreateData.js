...
class CreateData {
  constructor(data) {
    ...
    this.上限招股价 = 上限招股价;
    this.公开发售占比 = 公开发售占比;
    this.公开配售倍数 = 公开配售倍数.toFixed(2) - 0;
    this.国际配售倍数 = 国际配售倍数;
    this.上市日期 = 上市日期;
    this.股票代号 = 股票代号;
    this.创业板 = 创业板;
    ...
    this.甲组申购金额 = parseInt(
      (this.公开发售手数 / 2) * this.甲组申购倍数 * this.一手金额
    );
    this.乙组申购金额 = parseInt(
      (this.公开发售手数 / 2) * this.乙组申购倍数 * this.一手金额
    );
  ...
  计算一手占比(data) {
    ...
    if (data.创业板) {
      ...
      return (
        (((item.申购人数 * item.单手中签率) / data.公开发售手数) * 2).toFixed(
          4
        ) - 0
      );
    }
